...
public class HiveToTrinoConverterTest {
  ...
  public void testViews(String database, String view, String expectedSql) {
    ...
    RelToTrinoConverter relToTrinoConverter = new RelToTrinoConverter(hiveToRelConverter.getSqlValidator());
  ...
  public void testLegacyUnnestArrayOfStruct() {
    ...
    RelToTrinoConverter relToTrinoConverter = new RelToTrinoConverter(
        ImmutableMap.of(SUPPORT_LEGACY_UNNEST_ARRAY_OF_STRUCT, true), hiveToRelConverter.getSqlValidator());
    String expandedSql = relToTrinoConverter.convert(relNode);
    assertEquals(expandedSql, targetSql);
  }
  ...
  public void testAvoidTransformToDate() {
    ...
    RelToTrinoConverter relToTrinoConverter = new RelToTrinoConverter(
        ImmutableMap.of(AVOID_TRANSFORM_TO_DATE_UDF, true), hiveToRelConverter.getSqlValidator());
    String expandedSql = relToTrinoConverter.convert(relNode);
    assertEquals(expandedSql, targetSql);
  }
  ...
  public void testTypeCastForDataAddFunction() {
    RelToTrinoConverter relToTrinoConverter =
        new RelToTrinoConverter(ImmutableMap.of(CAST_DATEADD_TO_STRING, true), hiveToRelConverter.getSqlValidator());
  ...
  public void testConcatFunction() {
    ...
    String targetSql = "SELECT \"concat\"(CAST(CURRENT_DATE AS VARCHAR), '|', CAST(CURRENT_DATE AS VARCHAR), '-00')\n"
        + "FROM (VALUES  (0)) AS \"t\" (\"ZERO\")";
    String expandedSql = relToTrinoConverter.convert(relNode);
    assertEquals(expandedSql, targetSql);
  }
