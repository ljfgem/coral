...
/**
 * This transformer converts CAST(ROW: RECORD_TYPE) to named_struct
 * For example:
 * `CAST(ROW(123, 'xyz') AS ROW(`abc` INTEGER, `def` CHAR(3) CHARACTER SET `ISO-8859-1`))`
 * will be converted to `named_struct('abc', 123, 'def', 'xyz')`
 */
public class CastToNamedStructTransformer extends SqlCallTransformer {
  ...
  protected SqlCall transform(SqlCall sqlCall) {
    ...
    final SqlCall rowCall = (SqlCall) sqlCall.getOperandList().get(0);
    final SqlRowTypeSpec sqlRowTypeSpec = (SqlRowTypeSpec) sqlCall.getOperandList().get(1);
