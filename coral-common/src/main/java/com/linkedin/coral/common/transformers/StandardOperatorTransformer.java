...
/**
 * Standard operator transformer which can be applied to the input SqlNode directly without requiring the type derivation
 */
public class StandardOperatorTransformer extends OperatorTransformer {
  ...
  public boolean condition() {
    return inputSqlNode instanceof SqlCall
        && fromOperatorName.equalsIgnoreCase(((SqlCall) inputSqlNode).getOperator().getName())
        && ((SqlCall) inputSqlNode).getOperandList().size() == numOperands;
  }

  @Override
  public SqlNode transform() {
    List<SqlNode> sourceOperands = ((SqlCall) inputSqlNode).getOperandList();
